    ...
    for(int i=0; i<repeat; ++i) {
      ...
      for(long j=1; j < size_M-1; ++j){
        ...
        for(long i=1; i < size_N-1; ++i){
          long idx = j*size_N + i;
            b[idx] = c0 * (a[idx]
                + a[idx - 1] + a[idx - size_N]  // a[j][i-1] + a[j-1][i]
                + a[idx + size_N] + a[idx + 1]  // a[j+1][i] + a[j][i+1]
            );
        }
      }
    } 
...
{ 
  ...
  printf("2D5PT b[j][i] = c0 * (a[j][i] + a[j][i-1] + ...), 64 byte/it, 5 Flop/it\n");
